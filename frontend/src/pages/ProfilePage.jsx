    ...
    <section className="mx-auto max-w-4xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <header className="flex flex-col gap-4 rounded-2xl border border-neutral-200 bg-surface-raised p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-heading text-h3 font-bold text-heading">{name || 'User Profile'}</h1>
            <p className="text-small text-neutral-600">Manage your profile metrics and preferences.</p>
          </div>
        </div>

        <div className="flex items-center gap-4 rounded-xl border border-neutral-100 bg-surface-app px-4 py-3 shadow-inner">
          <div className="text-center">
            <span className="block text-small font-semibold text-primary uppercase tracking-wide">XP Points</span>
          ...
          <div className="text-center">
            <span className="block text-small font-semibold text-primary uppercase tracking-wide">Streak</span>
            <span className="font-heading text-h5 font-bold text-heading">
      ...
      <Card className="space-y-4">
        ...
        <form onSubmit={handleProfileSave} className="space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-small font-semibold text-heading block">Current Learning Goals</label>
            <textarea
              value={goals}
          ...
          <div className="space-y-1">
            <label className="text-small font-semibold text-heading block">Update Profile Picture</label>
            <input
              type="file"
        ...
        <select
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-500">Receive periodic update emails on missing course assignments</p>
            </div>
          </label>
        </div>
      </Card>
      ...
      <div className="rounded-2xl border-2 border-dashed border-red-300 bg-red-50/50 p-6 space-y-4">
        <header className="space-y-1">
          ...
          <p className="text-small text-neutral-600">
            Deleting your account triggers an irreversible data purge. You will immediately lose platform streaks, course rewards, and accumulated lesson progress records.
          </p>
        </header>

        <form onSubmit={handleDeleteAccount} className="space-y-3 max-w-md">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-neutral-700 block">Type account email to verify permanent extraction:</label>
            <input
              type="email"
              required
