...
function getQuizStyles(percent) {
  ...
  let leftColor = mixColor(
    quizColors.yellowLeft,
    quizColors.goldLeft,
    goldProgress,
  )

  let rightColor = mixColor(
    quizColors.yellowRight,
    quizColors.goldRight,
    goldProgress,
  )
...
export default function ProgressBar({
      ...
      return (
        <div className={`w-full ${className}`.trim()}>
          <div {...progressBarProps}>
            <div
              className={`relative ${imageWrapperClassName}`.trim()}
            >
  ...
  if (variant === 'circular') {
    const circleSize =
      circularSizes[size] || circularSizes.md
    ...
    return (
      ...
      >
        ...
        <span className="absolute text-sm font-semibold text-heading">
          {roundedPercent}%
        </span>
      </div>
    )
